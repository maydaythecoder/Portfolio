...
const ScrollAnimationTwo: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const elementRef = useRef<HTMLDivElement | null>(null);
  const [lastScrollTop, setLastScrollTop] = useState(window.pageYOffset || document.documentElement.scrollTop);
  const [inViewport, setInViewport] = useState(false);
  const maxDisplacement = 1800; // maximum travel distance
  const scrollFactor = 110 / 100; // 80px per 100px scrolled
  let ticking = false;
  ...
  return (
    <div
      className="scroll-animation-container mt-32"
      ref={elementRef}
      style={{
