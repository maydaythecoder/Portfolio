...
const ScrollAnimationThree: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const elementRef = useRef<HTMLDivElement | null>(null);
  const [lastScrollTop, setLastScrollTop] = useState(window.pageYOffset || document.documentElement.scrollTop);
  const [inViewport, setInViewport] = useState(false);
  const maxDisplacement = 1400; // maximum travel distance
  const scrollFactor = 80 / 100; // 80px per 100px scrolled
  let ticking = false;
...
export default ScrollAnimationThree;